...
def produceOneFigure(groundTruthDF,predictedDF,shop_id,title,copy_path,view_ax):
    '''
     绘制预测和真实值两者的序列
...
def produceOneGapFigure(groundTruthDF,predictedDF,shop_id,title,copy_path,view_ax):
    '''
     绘制差值序列
    :param groundTruthDF: 现实数据的真实值数据框
    :param predictedDF:   预测出来的数据的数据集框
...
def produceOneScoreFigure(groundTruthDF,predictedDF,shop_id,title,copy_path,view_ax):
    '''
     绘制分数序列
    :param groundTruthDF: 现实数据的真实值数据框
    :param predictedDF:   预测出来的数据的数据集框
...
def GenerateAllContrastFig(groundTruthDF,predictedDF,shopDF,figtype):
    '''
    ...
    for id_item in ID_list:
        ...
        if str(_cur_shop_info.cate1_name.values[0]) == '美食':
             copy_path = 'contrast_figure\\food_score_f4\\'+str(id_item)+'_contrastscore.png';
        else:
            if str(_cur_shop_info.cate1_name.values[0]) == '超市便利店':
                copy_path = 'contrast_figure\\market_score_f4\\'+str(id_item)+'_contrastscore.png'
            else:
                copy_path = 'contrast_figure\\other_score_f4\\'+str(id_item)+'_contrastscore.png'
        if figtype==0:
           produceOneFigure(groundTruthDF,predictedDF,id_item,title,copy_path,view_ax)
        if figtype == 1:
            produceOneGapFigure(groundTruthDF, predictedDF, id_item, title, copy_path, view_ax)
        if figtype == 2:
            produceOneScoreFigure(groundTruthDF, predictedDF, id_item, title, copy_path, view_ax)
...
if __name__=='__main__':
    ...
    GenerateAllContrastFig(train_data,predict_data,shop_info,2)