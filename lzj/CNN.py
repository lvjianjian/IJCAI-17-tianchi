...
import numpy as np
from keras.layers import Dense,Lambda,Convolution1D,Flatten,AveragePooling1D,MaxPooling1D,Dropout
...
def create_model_CNN(input_shape, h1_unit = 16, optimizer = "adagrad", init = "normal",h1_activation="relu"):
    # create model
    model = Sequential()
    model.add(Convolution1D(h1_unit, 3, border_mode="valid", input_shape=input_shape,activation=h1_activation))
    model.add(Convolution1D(h1_unit, 3, border_mode="valid", input_shape=input_shape,activation=h1_activation))
    model.add(AveragePooling1D())
    model.add(Flatten())
    # model.add(Dropout(0.5))
    model.add(Dense(1, init=init, activation='linear', activity_regularizer=activity_l2(0.015)))#
    # Compile model
...
def predictAllShop_CNN_HPS(all_data, trainAsTest=False, saveFilePath = None, featurePath = None,
    ...
    if model_path is None:
        if needCV:
            ...
            h1_acqtivation = ["relu"]
            h1_unit = [8, 12, 16, 20]
            model = KerasRegressor(build_fn=create_model_CNN, verbose=verbose)
            batch_size = [3, 5, 7, 10]
            epochs = [10, 15, 20, 25, 30]
            param_grid = dict(batch_size = batch_size, nb_epoch = epochs, h1_unit = h1_unit, input_shape = input_dim,h1_activation = h1_acqtivation)
        ...
        best_model = create_model_CNN(input_shape=input_dim, h1_unit=h1_unit, h1_activation = h1_activation)
...
def CNN_N_predict(n, trainAsTest, final_save_file):
    ...
    prefs = []
    for i in range(n):
        p = predictAllShop_CNN_HPS(payinfo, trainAsTest, Parameter.projectPath + "result/CNN_rt_hps",
...
if __name__ == "__main__":
    ...
    # CNN_N_predict(1, True, Parameter.projectPath + "lzj/final_result/CNN_ms")
    fe_path = None
    model_path = None
    predictAllShop_CNN_HPS(payinfo, True, Parameter.projectPath + "result/CNN_rt_hps",
                            fe_path, needSaveFeature=True, needCV=False,
                            cate_level=1, cate_name="美食",
                            ignore_shopids=[5, 125, 284, 381, 411, 416, 434, 437, 444, 459, 470, 474, 501, 521, 524, 530, 619, 632, 654,
                                             659, 660, 683, 700, 721, 727, 735, 742, 752,
