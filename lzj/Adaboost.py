...
def predictAllShop_AdaBoost_HPS(all_data, trainAsTest=False, saveFilePath = None, featurePath = None,
    ...
    if model_path is None:
        if needCV:
            ...
            param_grid = dict(n_estimators = n_estimators,learning_rate=learning_rate,
                              base_estimator=base_estimator,loss=loss)

            grid = GridSearchCV(estimator=AdaBoostRegressor(), param_grid=param_grid, n_jobs=-1, scoring="neg_mean_squared_error",verbose=2)
            grid.refit = False
        ...
        if not needCV:
            ...
            base_estimator = grid_result.best_params_['base_estimator']
        ...
        best_model = AdaBoostRegressor(base_estimator=base_estimator,n_estimators=n_estimators,learning_rate=learning_rate,loss=loss)
        ...
        if trainAsTest:
            model_save_path = Parameter.projectPath+"lzj/train_model/" + \
                              ...
                              % (last_N_days, sameday_backNum, day_back_num, train_x.shape[1], cate_level, cate_name
                                 , n_estimators, learning_rate, -1, loss)
    ...
    if saveFilePath is not None:
        if model_path is None:
            path = saveFilePath + "%dLast_%ds_%dd_%df_%d_%s_%d_%d_%d_%s_%dshops" \
                                  % (last_N_days,sameday_backNum, day_back_num, train_x.shape[1], cate_level, cate_name
                                     ,  n_estimators, learning_rate, -1, loss,len(shopids))
        else:
            import re
...
if __name__ == "__main__":
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=DeprecationWarning)
        predictAllShop_AdaBoost_HPS(payinfo, False, Parameter.projectPath + "result/AdaBoost_rt_hps",
                          ...
                          ignore_shopids= [5, 125, 284, 381, 411, 416, 434, 437, 444, 459, 470, 474, 501, 521, 524, 530, 619, 632, 654,
                                        ...
                                        768, 810, 956, 987, 1050, 1058, 1100, 1107, 1145, 1163, 1185, 1214, 1241,
                                        1243, 1380, 1384, 1407, 1447, 1462, 1464, 1486, 1510, 1526, 1548, 1556, 1567, 1609,
                                        1650, 1681, 1716, 1730, 1747, 1769, 1803, 1824, 1831, 1835, 1856, 1858, 1893, 1918, 1968]
