...
import numpy as np
from keras.layers import Dense,Lambda,Convolution1D,Flatten,AveragePooling1D,MaxPooling1D,Dropout,Merge
...
def create_model_MultiCNN(input_shape1,input_shape2 = None, h1_unit = 16, optimizer ="adagrad", init ="normal", h1_activation="relu"):
    # create model
    model = Sequential()
    model.add(Convolution1D(h1_unit, 3, border_mode="valid", input_shape=input_shape1, activation=h1_activation))
    model.add(Convolution1D(h1_unit, 3, border_mode="valid", activation=h1_activation))
    ...
    model2 = Sequential()
    model2.add(Convolution1D(h1_unit, 2, border_mode="valid",input_shape=input_shape2, activation=h1_activation))
    model2.add(Convolution1D(h1_unit, 2, border_mode="valid", activation=h1_activation))
    model2.add(Flatten())
    # model2.add(Dense(1, init=init, activation='linear', activity_regularizer=activity_l2(0.01)))

    final_model = Sequential()
    final_model.add(Merge([model, model2],mode="concat", concat_axis=1))
    final_model.add(Dense(1, init=init,activation='linear'))
...
def predictAllShop_MultiCNN_HPS(all_data, trainAsTest=False, saveFilePath = None, featurePath = None,
    ...
    if featurePath is None:
        train_x, train_y = getTrainXY(all_data, cate1_hot_encoder, cate1_label_encoder, day_back_num, hot_encoder,
                                         ignore_get_train, ignore_shopids, last_N_days, other_features, 0,
                                         shop_features, shop_info, shopids, trainAsTest, weather, weekOrWeekend,
                                         week_backnum)
        train_x2 = getTrainXY(all_data, cate1_hot_encoder, cate1_label_encoder, 0, hot_encoder,
                              ignore_get_train, ignore_shopids, last_N_days, other_features, sameday_backNum,
                              shop_features, shop_info, shopids, trainAsTest, weather, weekOrWeekend,
                              week_backnum)[0]

        """增广训练集"""
        if Augmented:
            train_xs, train_y = augmentTrainX(augument_time, [train_x, train_x2], train_y)
    ...
    train_x = np.reshape(train_x, (train_x.shape[0],
                                  train_x.shape[1], 1))
    train_x2 = np.reshape(train_x2, (train_x2.shape[0],
                                   train_x2.shape[1], 1))
    ...
    if model_path is None:
        if needCV:
            ...
            h1_acqtivation = ["relu"]
            h1_unit = [8, 12, 16, 20]
            model = KerasRegressor(build_fn=create_model_MultiCNN, verbose=verbose)
            batch_size = [3, 5, 7, 10]
            epochs = [10, 15, 20, 25, 30]
            param_grid = dict(batch_size = batch_size, nb_epoch = epochs, h1_unit = h1_unit, input_shape1 = input_dim, input_shape2 = input_dim2)
        ...
        best_model = create_model_MultiCNN(input_shape1=input_dim,input_shape2=input_dim2, h1_unit=h1_unit, h1_activation = h1_activation)
        hist = best_model.fit([train_x,train_x2], train_y, verbose=verbose, batch_size=batch_size, nb_epoch=epochs,  validation_split = 0.1, callbacks=[early_stopping])
        print hist.history
    ...
    for j in shopids:
        ...
        for i in range(14):
            ...
            x = None
            x2 = None
            x = getOneShopTrainXY(cate1_hot_encoder, cate1_label_encoder, day_back_num, hot_encoder, other_features, part_data
                                  , 0, shop_features, shop_info, j, part_data.shape[0] - 1, x
                                  , weather, weekOrWeekend, week_backnum)[0]
            x2 = getOneShopTrainXY(cate1_hot_encoder, cate1_label_encoder, 0, hot_encoder, other_features, part_data
                                   , sameday_backNum, shop_features, shop_info, j, part_data.shape[0] - 1, x2
                                   , weather, weekOrWeekend, week_backnum)[0]
...
def augmentTrainX(augument_time, train_xs, train_y):
    print "augment data"
    new_train_xs = []
    if not isinstance(train_xs,list):
        train_xs = [train_xs]
    for i in range(len(train_xs)):
        new_train_xs.append(np.ndarray((train_xs[i].shape[0] * (augument_time + 1), train_xs[i].shape[1])))
    ...
    end = train_xs[0].shape[0]
    for index in range(end):
        for i in range(len(new_train_xs)):
            new_train_xs[i][index] = train_xs[i][index]
    ...
    for index in range(end):
        ...
        for t in range(augument_time):
            for i in range(len(new_train_xs)):
                new_train_xs[i][sert_index] = train_xs[i][index]
    ...
    return new_train_xs, new_train_y


def getTrainXY(all_data, cate1_hot_encoder, cate1_label_encoder, day_back_num, hot_encoder, ignore_get_train,
               ignore_shopids, last_N_days, other_features, sameday_backNum, shop_features, shop_info, shopids,
               trainAsTest, weather, weekOrWeekend, week_backnum):
    all_x = None
    all_y = None
    for shopid in shopids:
        ...
        if skipNum < 0:
            skipNum = 0
        train_x = None
        train_x, train_y = getOneShopTrainXY(cate1_hot_encoder, cate1_label_encoder, day_back_num, hot_encoder,
                                             other_features, part_data, sameday_backNum, shop_features, shop_info,
                                             shopid, skipNum, train_x, weather, weekOrWeekend, week_backnum)
...
def getOneShopTrainXY(cate1_hot_encoder, cate1_label_encoder, day_back_num, hot_encoder, other_features, part_data,
                      sameday_backNum, shop_features, shop_info, shopid, skipNum, train_x, weather, weekOrWeekend,
                      week_backnum):
    '''获取特征'''
    ...
    if day_back_num != 0:  # day
        if train_x is not None:
            train_x = np.concatenate((train_x, getOneWeekdayFomExtractedData(
                extractBackDay(part_data, day_back_num, skipNum, nan_method_sameday_mean))), axis=1)
        else:
            train_x = getOneWeekdayFomExtractedData(
                extractBackDay(part_data, day_back_num, skipNum, nan_method_sameday_mean))
    ...
    for feature in other_features:
        value = getOneWeekdayFomExtractedData(
            extractBackWeekValue(part_data, week_backnum, skipNum, nan_method_sameday_mean, feature))
...
def MultiCNN_N_predict(n, trainAsTest, final_save_file):
    ...
    prefs = []
    for i in range(n):
        p = predictAllShop_MultiCNN_HPS(payinfo, trainAsTest, Parameter.projectPath + "result/CNN_rt_hps",
...
if __name__ == "__main__":
    ...
    # MultiCNN_N_predict(1, True, Parameter.projectPath + "lzj/final_result/CNN_ms")
    fe_path = None
    model_path = None
    predictAllShop_MultiCNN_HPS(payinfo, True, Parameter.projectPath + "result/MultiCNN_rt_hps",
                            fe_path, needSaveFeature=True, needCV=False,
                            cate_level=1, cate_name="美食",
                            ignore_shopids=[5, 125, 284, 381, 411, 416, 434, 437, 444, 459, 470, 474, 501, 521, 524, 530, 619, 632, 654,
                                             659, 660, 683, 700, 721, 727, 735, 742, 752,
                                             ...
                                             1243, 1380, 1384, 1407, 1447, 1462, 1464, 1486, 1510, 1526, 1548, 1556, 1567, 1609,
                                             1650, 1681, 1716, 1730, 1747, 1769, 1803, 1831, 1835, 1856, 1858, 1893, 1918, 1968]
                            ,Augmented=True, model_path=model_path, ignore_predict=True)
