...
from NNHPSearch import getModel,saveModel
...
def create_model_lstm(input_dim,h_unit = 24,h_activation="sigmoid"):
    model = Sequential()
    model.add(LSTM(h_unit, input_shape=input_dim, activation=h_activation))
    model.add(Dense(1, init="normal", activation='linear', activity_regularizer=activity_l2(0.01)))
    model.compile(loss="mse",optimizer="rmsprop")
    return model

def predictAllShop_LSTM_HPS(all_data, trainAsTest=False, saveFilePath = None, featurePath = None,
    ...
    if model_path is None:
        if needCV:
            ...
            h1_activation = ["relu"]
            h1_unit = [8,12,16,20]
            model = KerasRegressor(build_fn=create_model_lstm, verbose=verbose)
            batch_size = [3,5,7,10]
            epochs = [10,15,20,25,30]
            param_grid = dict(batch_size = batch_size, nb_epoch = epochs, h_unit = h1_unit, input_dim = input_dim)
        ...
        if not needCV:
            ...
            h1_unit = grid_result.best_params_["h_unit"]
            h1_activation = "sigmoid"

        early_stopping = EarlyStopping(monitor='val_loss', patience=2)
        best_model = create_model_lstm(input_dim=input_dim, h_unit=h1_unit, h_activation = h1_activation)
        ...
        if trainAsTest:
            model_save_path = Parameter.projectPath+"lzj/train_model/" + \
                              "%dlast_%ds_%dd_%df_%d_%s_%d_%d_%d_%s_lstm.json" \
    ...
    if saveFilePath is not None:
        path = saveFilePath + "%dLast_%ds_%dd_%df_%d_%s_%d_%d_%d_%s_%dshops" \
                              % (last_N_days,sameday_backNum, day_back_num, train_x.shape[1], cate_level, cate_name
                                 ,  epochs, batch_size, h1_unit, h1_activation,len(shopids)-ignores)
        if Augmented:
            path += "_augmented"
        if trainAsTest:
...
if __name__ == "__main__":
    ...
    # model_path = Parameter.projectPath + "lzj/train_model/70last_7s_0d_7f_1_超市便利店_40_3_10_sigmoid.json"
    model_path = None
    predictAllShop_LSTM_HPS(payinfo, True, Parameter.projectPath + "result/LSTM_rt_hps",
                            fe_path, needSaveFeature=True, needCV=False,
                            cate_level=1, cate_name="美食",
                            ignore_shopids=[5, 125, 284, 381, 411, 416, 434, 437, 444, 459, 470, 474, 501, 521, 524, 530, 619, 632, 654, 659, 660, 683, 700, 721, 727, 735, 742, 752, 768, 810, 956, 1050, 1058, 1100, 1107, 1145, 1163, 1185, 1214, 1241, 1243, 1380, 1384, 1407, 1447, 1462, 1464, 1486, 1510, 1526, 1548, 1556, 1567, 1609, 1650, 1681, 1716, 1730, 1747, 1769, 1803, 1831, 1835, 1856, 1858, 1893, 1918, 1968], Augmented=False, model_path=model_path)
