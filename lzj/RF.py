...
def predictAllShop_RF_HPS(all_data, trainAsTest=False, saveFilePath = None, featurePath = None,
    ...
    if model_path is None:
        if needCV:
            ...
            max_leaf_nodes = [None]
            param_grid = dict(n_estimators = n_estimiter,max_features=max_features,
                              max_depth=max_depth,min_samples_split=min_samples_split,min_samples_leaf=min_samples_leaf,
                              min_weight_fraction_leaf=min_weight_fraction_leaf,max_leaf_nodes=max_leaf_nodes)

            grid = GridSearchCV(estimator=RandomForestRegressor(n_jobs=-1), param_grid=param_grid, n_jobs=1, scoring="neg_mean_squared_error",verbose=2)
            grid.refit = False
        ...
        if not needCV:
           ...
           max_leaf_nodes = None
            ...
            n_estimiter = grid_result.best_params_['n_estimators']
            max_features = grid_result.best_params_['max_features']
            max_depth = grid_result.best_params_['max_depth']
            min_samples_split = grid_result.best_params_['min_samples_split']
            min_samples_leaf = grid_result.best_params_['min_samples_leaf']
            min_weight_fraction_leaf = grid_result.best_params_['min_weight_fraction_leaf']
            max_leaf_nodes = grid_result.best_params_['max_leaf_nodes']



        best_model = RandomForestRegressor(criterion="mse",n_jobs=1,n_estimators=n_estimiter,max_features=max_features,
                                           max_depth=max_depth,min_samples_split=min_samples_split,min_samples_leaf=min_samples_leaf,
                                           min_weight_fraction_leaf=min_weight_fraction_leaf,max_leaf_nodes=max_leaf_nodes)
        ...
        if trainAsTest:
            model_save_path = Parameter.projectPath+"lzj/train_model/" + \
                              ...
                              % (last_N_days, sameday_backNum, day_back_num, train_x.shape[1], cate_level, cate_name
                                 , n_estimiter, min_samples_leaf, min_samples_split, max_features)
    ...
    if saveFilePath is not None:
        if model_path is None:
            path = saveFilePath + "%dLast_%ds_%dd_%df_%d_%s_%d_%d_%d_%s_%dshops" \
                                  % (last_N_days,sameday_backNum, day_back_num, train_x.shape[1], cate_level, cate_name
                                     , n_estimiter, min_samples_leaf, min_samples_split, max_features,len(shopids))
        else:
            import re
...
if __name__ == "__main__":
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=DeprecationWarning)
        predictAllShop_RF_HPS(payinfo, False, Parameter.projectPath + "result/RF_rt_hps",
                          fe_path, needSaveFeature=True, needCV=False,
                          cate_level=1, cate_name="超市便利店",
                          ignore_shopids= [23, 627, 749, 1269, 1218]
                          ,Augmented=False, model_path=model_path, ignore_predict=False)
